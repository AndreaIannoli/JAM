...
export class MosquittoConnector implements BrokerConnector {
    ...
    public async connect(conf: BrokerConf) {
        return new Promise((resolve, reject) => {
			this.client  = mqtt.connect(conf.url,{clientId: conf.id, protocol: "ws", protocolVersion: 4, connectTimeout:5000, debug:true, username: conf.username, password: conf.password});
